        ...
        >
            @foreach($roles as $role)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                    <th class="px-4 py-1 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $role->name }}</th>
                    <td class="px-4 py-1 ">{{ $role->description ?? '-' }}</td>
                    <td class="px-4 py-1 ">{{ $role->created_at ? $role->created_at->format('d-m-Y') : ''  }}</td>
                    <td class="px-4 py-1 ">
                        <x-ui.button-actions 
