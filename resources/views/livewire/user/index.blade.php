        ...
        >
            @foreach($users as $user)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                    <th class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $user->name }}</th>
                    <td class="px-4 py-2 ">{{ $user->username }}</td>
                    <td class="px-4 py-2 ">{{ $user->role->name }}</td>
                    <td class="px-4 py-2 ">{{ $user->created_at ?? ''  }}</td>
                    <td class="px-4 py-2 ">
                        <x-ui.button-actions 
                            :urlAction="route('users.edit', $user->id)"
                            :editAction="'edit(' . $user->id . ')'"
                            :deleteAction="'delete(' . $user->id . ')'"
                        />    
                    </td>
                </tr>
            @endforeach
